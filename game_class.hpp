...
protected:
	pair<int, int> trans(int x, int y) const {
...
public:
	virtual bool legal_push_line(int x, int y, int direction, char OX) const {
		auto [cx, cy] = trans(x, y);
...
public:
	virtual bool legal_push_side(int sx, int sy, int line_direction, int push_direction, int amount, char OX) const {
		auto [cx, cy] = trans(sx, sy);
...
public:
	virtual vector<ActType> legal_moves(char OX) const {
		vector<ActType> result;
		for (int x = 1; x <= 9; x++) {
			for (int y = 1; y <= 9; y++) {
				auto [cx, cy] = trans(x, y);
				if (cx >= 11 || cy >= 11 || cx <= 0 || cy <= 0 || board[cx][cy] != OX) continue;
				...
				for (int ld = 0; ld < 6; ld++)
					for (int pd = 0; pd < 6; pd++)
						for (int amount = 1; amount <= 3; amount++)
							if (legal_push_side(x, y, ld, pd, amount, OX))
								result.push_back(ActType(x, y, ld, pd, amount));
			}
		}
		return result;
	}
...
public:
	void move(const ActType &act, char OX) {
		if (act.type == false)
			push_line(act.x, act.y, act.direction, OX);
		else
			push_side(act.x, act.y, act.line_direction, act.push_direction, act.amount, OX);
	}
};
