...
//  err 15,15,15,15,15;
    ...
    move(const int &x1,const int &y1,const int &x2,const int &y2,const int &direction):x1(x1),y1(y1),x2(x2),y2(y2),direction(direction){
        if(x1!=15&&y1!=15&&x2!=15&&y2!=15&&direction!=15) assert(legal_point(x1,y1)&&legal_point(x2,y2)&&legal_direction(direction));
    }
    move(const int &val):x1(val&15),y1(val>>4&15),x2(val>>8&15),y2(val>>12&15),direction(val>>16&15){
        if(val==-1) x1=y1=x2=y2=direction=15;
        else assert(legal_point(x1,y1)&&legal_point(x2,y2)&&legal_direction(direction));
    }
    move():x1(15),y1(15),x2(15),y2(15),direction(15){};
    inline int encode(){
            ...
            for(int x=0;x<9;x++){
                ...
                for(int direction=0,xn,yn,xnn,ynn;direction<6;direction++){
                    ...
                    if(board[xn][yn]==0){
                        ...
                        moves.push_back(tmp.encode()); //move 1
                    }
                    else if(direction<3&&board[xn][yn]==current_player){
                        for(int ndirection=0,nx,nxn,ny,nyn,nxnn,nynn,tmpx,tmpy;ndirection<6;ndirection++){
                            if(ndirection==direction){
                                ...
                                else if(board[nxn][nyn]==current_player){// 3
                                    xnn=nxn,ynn=nyn;
                                    nxnn=xnn+dx[ndirection],nynn=ynn+dy[direction];
                                    success=false;
                            ...
                            else if(ndirection==direction+3||ndirection+3==direction){
                                nx=x+dx[ndirection],ny=y+dy[ndirection];
                                success=false;
                                if(!legal_point(nx,ny)) continue;
                                if(board[nx][ny]==0) success=true;
                                else if(board[nx][ny]!=current_player){
                                    tmpx=nx+dx[ndirection],tmpy=ny+dy[ndirection];
                                    success=false;
                                    if(!legal_point(tmpx,tmpy)) success=true;
                                    else if(board[tmpx][tmpy]==0) success=true;
                                }
                                ...
                                if(board[xnn][ynn]==current_player&&board[nxnn][nynn]==0){
                                    tmp=move(x,y,xnn,ynn,ndirection);
                                    moves.push_back(tmp.encode());
                                }
                            }
